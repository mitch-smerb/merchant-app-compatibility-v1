...
export function StatCard({ title, value, insight, insightColor = 'blue', children, dataId }: StatCardProps) {
  return (
    <div className="bg-card shadow-sm border border-border rounded-[16px]" data-id={dataId}>
      <div className="pb-2 px-8 pt-8">
        <h4 className="text-foreground text-[18px]">{title}</h4>
      </div>
      <div className={`${children ? "pt-12" : "pt-4"} px-8 pb-8`}>
        {value && (
          <div className="mb-6">
            <h1 className="text-foreground tabular-nums">{value}</h1>
          </div>
        )}
        {children}
        {insight && (
