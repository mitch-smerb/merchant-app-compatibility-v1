...
interface BusinessRegistrationFormHookParams {
  ...
  schemaUpgrade?: Joi.ObjectSchema<any>;
}
...
export const useBusinessRegistrationForm = ({
  ...
  const useThisSchema = schemaUpgrade || schema;
  ...
  useEffect(() => {
    ...
    const isColumbiacuRoute = window.location.pathname.includes('/columbiacu');
    if (isColumbiacuRoute)  {
      setValue(
        'billingInfo.promoCode',
        PROMO_CODE_COLUMBIACU
      );
    }
  }, [promoCodeAppliedMembership]);
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    setValidationErrors([]);
    handleSubmitBase(handleSignup)(e);
  };
